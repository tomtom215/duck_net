...
| **OData** | `odata_query`, `odata_paginate` (table), `odata_extract_count`, `odata_metadata` (table) | OData queries with $filter/$select/$top/$orderby/$expand; automatic pagination via @odata.nextLink (v4) and __next (v2 JSON); total count extraction; $metadata CSDL introspection for entity/property discovery (v4 and v2/v3) |
