...
quack_rs::scalar_callback!(cb_jsonrpc_call_3, |_info, input, output| {
    let chunk = unsafe { DataChunk::from_raw(input) };
    let row_count = chunk.size();
    let url_reader = unsafe { chunk.reader(0) };
    let method_reader = unsafe { chunk.reader(1) };
    let params_reader = unsafe { chunk.reader(2) };
...
quack_rs::scalar_callback!(cb_jsonrpc_call_4, |_info, input, output| {
    let chunk = unsafe { DataChunk::from_raw(input) };
    let row_count = chunk.size();
    let url_reader = unsafe { chunk.reader(0) };
    let method_reader = unsafe { chunk.reader(1) };
...
quack_rs::scalar_callback!(cb_xmlrpc_call, |_info, input, output| {
    let chunk = unsafe { DataChunk::from_raw(input) };
    let row_count = chunk.size();
    let url_reader = unsafe { chunk.reader(0) };
    let method_reader = unsafe { chunk.reader(1) };
    ...
    for row in 0..row_count {
        let url = unsafe { url_reader.read_str(row) };
        let method = unsafe { method_reader.read_str(row) };
        let params_str = unsafe { params_reader.read_str(row) };
